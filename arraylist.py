class ArrayList:
    ...
    def __str__(self):
        return "[" + ", ".join(repr(x) for x in self._data[:self._count]) + "]"
...
if __name__ == "__main__":
    array_list = ArrayList(1, 2, 3)
    assert str(array_list) == "[1, 2, 3]"
    assert array_list == [1, 2, 3]
    assert array_list[0] == 1

    array_list.append(4)
    assert array_list == [1, 2, 3, 4]

    array_list.remove(1)
    assert array_list == [1, 3, 4]

    array_list.remove(2)
    assert array_list == [1, 3]

    for i in range(1000):
        array_list.append(i)
    assert len(array_list) == 1002